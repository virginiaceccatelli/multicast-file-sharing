...
mcast_t *multicast_init(char *mcast_addr, int sport, int rport)
{
    ...
    // Allow port reuse
    int optval = 1;
    if (setsockopt(m->sock, SOL_SOCKET, SO_REUSEPORT, &optval, sizeof(optval)) < 0) {
        perror("setsockopt SO_REUSEPORT");
    }
    if (setsockopt(m->sock, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval)) < 0) {
        perror("setsockopt SO_REUSEADDR");
        exit(1);
    }

    // Enable multicast loopback so sender/receiver on same machine can work
    unsigned char loop = 1;
    if (setsockopt(m->sock, IPPROTO_IP, IP_MULTICAST_LOOP, &loop, sizeof(loop)) < 0) {
