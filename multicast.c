...
// Creates and initializes the multicast object.
mcast_t *multicast_init(char *mcast_addr, int sport, int rport)
{
    mcast_t *m = (mcast_t *)calloc(1, sizeof(mcast_t));

    // Create UDP socket
    m->sock = socket(AF_INET, SOCK_DGRAM, 0); // AF_INET -> IPv4,  SOCK_DGRAM -> UDP
    if (m->sock < 0)
    {
        perror("socket");
        exit(1);
    ...
    // Allow port reuse, so multiple processes can bind the same port
    int optval = 1;
    ...
    // Set multicast destination (send target address and port)
    ...
    // Set local receiving address (and port)
    m->my_addr.sin_addr.s_addr = htonl(INADDR_ANY); // INADDR_ANY -> receive on any local interface
    ...
    // Setup poll(), lets you check if data is available without blocking
